...
local mcMasterModule = {}

function mcMasterModule.WriteRegister(name, val)
...
--function mcMasterModule.mcRegAddDel(hInst, mode, device, path, desc, intialVal, persistent) --"mode = ADD" or "DEL", 
function mcMasterModule.mcRegAddDel(inst, mode, device, path, desc, intialVal, persistent) --"mode = ADD" or "DEL", 
	local hReg
	local rc
...
--function mcMasterModule.doRegTable(mode, t, device, group)
function mcMasterModule.doRegTable(inst, mode, t, device, group)
	...
	for v = 1, #t do
		...
		--response, rc = mcMasterModule.mcRegAddDel(hInst, mode, device, path, desc, intialVal, persistent) --"ADD" or "DEL",
		response, rc = mcMasterModule.mcRegAddDel(inst, mode, device, path, desc, intialVal, persistent) --"ADD" or "DEL",
...
function mcMasterModule.GetRegister(regname, num)
...
function mcMasterModule.SetRegister(regname, val, num)
	local hreg = mc.mcRegGetHandle(inst, string.format("iRegs0/%s", regname))
	if (num == 1) then
...
function mcMasterModule.SaveRegister(ini, name)
    local hreg = mc.mcRegGetHandle(inst, string.format("iRegs0/%s", name))
    local val = mc.mcRegGetValueString(hreg)
    mc.mcProfileWriteString(0, tostring(ini), name, val)
end

function mcMasterModule.LoadRegister(ini, name)
	local val = mc.mcProfileGetString(inst , tostring(ini), name, "0.000")
	local hreg = mc.mcRegGetHandle(inst, string.format("iRegs0/%s", name))
	mc.mcRegSetValueString(hreg, val)
end

function mcMasterModule.SaveRegisters()
	--Auto tool setter registers
	--mcMasterModule.SaveRegister("AutoTool", "TS_XPos")
	--mcMasterModule.SaveRegister("AutoTool", "TS_YPos")
	--mcMasterModule.SaveRegister("AutoTool", "TS_TouchPos")
	--mcMasterModule.SaveRegister("AutoTool", "TS_ProbeH")
	--mcMasterModule.SaveRegister("AutoTool", "TS_DefaultL")
	--mcMasterModule.SaveRegister("AutoTool", "TS_Retract")
	--mcMasterModule.SaveRegister("AutoTool", "TS_GCode")
end

function mcMasterModule.LoadRegisters()
	--Auto tool setter registers
	--mcMasterModule.LoadRegister("AutoTool", "TS_XPos")
	--mcMasterModule.LoadRegister("AutoTool", "TS_YPos")
	--mcMasterModule.LoadRegister("AutoTool", "TS_TouchPos")
	--mcMasterModule.LoadRegister("AutoTool", "TS_ProbeH")
	--mcMasterModule.LoadRegister("AutoTool", "TS_DefaultL")
	--mcMasterModule.LoadRegister("AutoTool", "TS_Retract")
	--mcMasterModule.LoadRegister("AutoTool", "TS_GCode")
end

function mcMasterModule.ReturnCode()
...
function mcMasterModule.SimpleAutoZero()
-- Simple AutoZero
